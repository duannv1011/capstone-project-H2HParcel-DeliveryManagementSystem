...
export class OrderService {
    ...
    async getAllOrdersSeacrhNoPaging(accId: number, orderStatus: number): Promise<any> {
        ...
        switch (Number(orderStatus)) {
            case 2:
                queryBuilder.andWhere('o.order_stt BETWEEN :start AND :end', { start: 1, end: 2 });
                break;

            case 3:
                queryBuilder.andWhere('o.order_stt BETWEEN :start AND :end', { start: 3, end: 8 });
                break;

            case 4:
                queryBuilder.andWhere('o.order_stt = :orderStatus', { orderStatus: 9 });
                break;
    ...
    async getDetailOrder(orderId: number, accId: number): Promise<any> {
        ...
        const order = dataQuery
            ? {
                  ...
                  packeId: dataQuery.pkId,
                  packeType: dataQuery.packageType.pkName,
